...
void	cast_horz(t_data	*data)
{
	...
	first_y_inter = floor(data->player.pos_px.y / \
			data->my_cubs_len) * data->my_cubs_len;
	if (is_down(data))
		first_y_inter += data->my_cubs_len;
	first_x_inter = data->player.pos_px.x + ((first_y_inter - \
	...
	if (is_up(data))
		y_check--;
	while (((x_check >= 0 && x_check < ((data->max.x * \
							data->my_cubs_len)) && (y_check >= 0 \
							&& y_check < (data->max.y * data->my_cubs_len)))))
	{
...
void	cast_ver(t_data	*data)
{
	...
	first_x_inter = floor(data->player.pos_px.x \
			/ data->my_cubs_len) * data->my_cubs_len;
	if (is_right(data))
		first_x_inter += data->my_cubs_len;
	first_y_inter = data->player.pos_px.y + ((first_x_inter \
	...
	if (is_left(data))
		x_check--;
	while (((x_check >= 0 && x_check <= \
					((data->max.x * data->my_cubs_len) + 1) \
					&& (y_check >= 0 && y_check <= (data->max.y \
							* data->my_cubs_len) + 1))))
	{
		if (is_wall(data, y_check, x_check))
		{
